...
impl Track {
    ...
    pub fn send(&mut self, send_index: i32) -> &mut TrackSend {
        self.send_index_map.entry(send_index).or_insert_with(|| {
            TrackSend::new(self.socket.clone(), self.track_guid.clone(), send_index)
        })
    }
    ...
    F: Fn(&str),
{
    ...
    if let Some(args) = match_addr(addr, "/track/{track_guid}/send/{send_index}/guid") {
        let send_index: i32 = args[1].parse().unwrap();
